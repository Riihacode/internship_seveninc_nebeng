...
class UpdatePaymentMethodUseCase @Inject constructor(
    ...
    suspend operator fun invoke(token: String, id: Int, request: UpdatePaymentMethodRequest): Flow<Result<PaymentMethod>> {
        return repository.updatePaymentMethod(token, id, request)
    }
}