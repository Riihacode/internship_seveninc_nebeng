...
import com.example.nebeng.feature_a_authentication.presentation.profile.screen_base.DeleteAccountScreen
import com.example.nebeng.feature_a_authentication.presentation.profile.screen_base.EditProfileScreen
import com.example.nebeng.feature_a_authentication.presentation.profile.screen_base.ProfileDetailScreen
import com.example.nebeng.feature_a_authentication.presentation.profile.screen_role.customer.ProfileCustomerScreenUi
import com.example.nebeng.feature_a_authentication.presentation.profile.screen_role.driver.ProfileDriverScreenUi
import com.example.nebeng.feature_a_authentication.presentation.profile.support_for_present.userlist.UserListScreen
...
//    val user = uiState.currentUser
    ...
    // ✅ butuh context utk startActivity + finish
...
//        startDestination = startDestination
        ...
        startDestination = if (userType == "driver") "profile_driver_main" else "profile_customer_main"
    ) {
...
//                state = state,
//                onSave = { token, id, req ->
//                    viewModel.updateProfile(token, id, req)
...
//        }
        composable("profile_customer_main") {
            ProfileCustomerScreenUi(
                ...
                onEditProfile =
                    {
                    Log.d("NAV_DEBUG", "Navigate to edit_profile")
//                    navController.navigate("edit_profile")
                ...
                onLogout = {
                    // 1️⃣ clear state auth (kalau mau)
                    viewModel.logout()

                    // 2️⃣ langsung pindah ke AuthActivity (login graph)
                    activity?.let { act ->
                        act.startActivity(
                            Intent(act, AuthActivity::class.java).apply {
        ...
        composable("edit_profile") {
            ...
            SimpleScaffoldWithBack(
                ...
                EditProfileScreen(
                    ...
                    state = uiState,
                    onSave = { token, userId, request ->
                        // 🔥 panggil API update profil
                        viewModel.updateProfile(token, userId, request)
...
private fun SimpleScaffoldWithBack(
    ...
    Scaffold(
        ...
        content(padding)   // <- ini yang benar
    }
}
