...
class CreateGoodsTransactionUseCase @Inject constructor(
    private val repository: GoodsTransactionRepository
) {
    suspend operator fun invoke(token: String, request: CreateGoodsTransactionRequest): Flow<Result<GoodsTransaction>> {
        return repository.createGoodsTransaction(token, request)
    }
}