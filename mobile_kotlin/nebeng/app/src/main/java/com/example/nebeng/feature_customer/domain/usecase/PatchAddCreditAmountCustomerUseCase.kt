...
class PatchAddCreditAmountCustomerUseCase @Inject constructor(
    private val repository: CustomerRepository
) {
    suspend operator fun invoke(token: String, id: Int, request: PatchAddCreditAmountCustomerRequest): Flow<Result<Customer>> {
        return repository.patchAddCredit(token, id, request)
    }
}