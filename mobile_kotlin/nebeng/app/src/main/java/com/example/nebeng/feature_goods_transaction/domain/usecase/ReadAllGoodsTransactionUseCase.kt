...
class ReadAllGoodsTransactionUseCase @Inject constructor(
    private val repository: GoodsTransactionRepository
) {
    suspend operator fun invoke(token: String): Flow<Result<List<GoodsTransaction>>> {
        return repository.getAllGoodsTransactions(token)
    }
}