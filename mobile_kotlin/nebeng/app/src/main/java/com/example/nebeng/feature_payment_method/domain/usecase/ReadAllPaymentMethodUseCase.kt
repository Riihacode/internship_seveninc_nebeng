...
class ReadAllPaymentMethodUseCase @Inject constructor(
    private val repository: PaymentMethodRepository
) {
    suspend operator fun invoke(token: String): Flow<Result<List<PaymentMethod>>> {
        return repository.getAllPaymentMethods(token)
    }
}