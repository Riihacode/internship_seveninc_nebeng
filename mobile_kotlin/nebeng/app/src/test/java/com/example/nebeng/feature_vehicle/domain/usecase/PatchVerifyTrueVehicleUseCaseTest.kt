...
class PatchVerifyTrueVehicleUseCaseTest {

    @Mock private lateinit var repository: VehicleRepository
    private lateinit var useCase: PatchVerifyTrueVehicleUseCase

    @Before
    fun setup() {
        MockitoAnnotations.openMocks(this)
        useCase = PatchVerifyTrueVehicleUseCase(repository)
    }
    ...
    fun `invoke emits Success`() = runTest {
        val token = "token123"
        val id = 1
        val vehicle = Vehicle(1, 10, "AB1234CD", "2021", "2025-11-06", "stnk.png", "Honda PCX", "Hitam", VehicleType.MOTOR, "pcx.png", true, null)

        whenever(repository.patchVerifyTrueVehicleById(token, id)).thenReturn(flow {
